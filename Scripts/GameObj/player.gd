...
# --- ADDED: Fix for Death Loop ---
var invulnerable = false 

func _ready() -> void:
	# 1. Set invulnerable FIRST before moving to prevent frame-1 deaths
	if Global.respawn_point != null:
		invulnerable = true 
		global_position = Global.respawn_point
		
		# 2. Force the Camera to snap to the new position immediately
		var cam = get_viewport().get_camera_2d()
		if cam:
			cam.global_position = global_position
			if cam.has_method("reset_smoothing"):
				cam.reset_smoothing() # Prevents the camera from "sliding" from the start
		
		# 3. Start the visual effect
		start_invulnerability(2.0) # Increased to 2 seconds for safet
...
func take_damage():
	# Fix: Don't take damage if already dead or invulnerable
	...
	if health <= 0:
		...
		play_hurt_animation()
		start_invulnerability(0.5) # Brief protection after getting hit
...
func die() -> void:
	...
	# --- ADDED: Fix for Crash (Null Value) ---
	var tree = get_tree() # Capture the tree reference NOW
	await tree.create_timer(2.0).timeout 
	
	if tree: # Check if it still exists
